    ...
    for resol in mesh:
        ...
        for p_case in [1,2]:
            if p_case == 1: # apply term
                ...
                phi_x = cos(pi*x[0]*x[1])*pi*x[1]
                phi_y = cos(pi*x[0]*x[1])*pi*x[0]

                phi = as_vector((phi_x,phi_y))

                outFile2 = open('app_ss_p=' + str(p) + '_Re='+str(Re)+ '_m=' + str(resol) +'.csv', mode)
                ...
                f = inner(grad(u_exact)*u_exact,v)*dx - 2.0*nu*inner(div(nablas(u_exact)),v)*dx + inner(grad(p_ex),v)*dx + inner(phi,v)*dx
                ...
                ufile = File('app_ss_results/Re='+str(Re)+'_p=' + str(p) + '_m=' + str(resol) + '/u.pvd')
                vfile = File('app_ss_results/Re='+str(Re)+'_p=' + str(p) + '_m=' + str(resol) + '/v.pvd')
            ...
            else:  # dont apply term

                outFile3 = open('ori_ss_p=' + str(p) + '_Re='+str(Re)+ '_m=' + str(resol) +'.csv', mode)
                ...
                ufile = File('ori_ss_results/Re='+str(Re)+'_p=' + str(p) + '_m=' + str(resol) + '/u.pvd')
                vfile = File('ori_ss_results/Re='+str(Re)+'_p=' + str(p) + '_m=' + str(resol) + '/v.pvd')
